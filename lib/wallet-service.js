...
function buildActiveOrderFlowKey({ buyerUserId, sellerUserId, adId, asset = 'USDT', type = '' } = {}) {
  return [
    ensureUserId(buyerUserId, 'buyerUserId'),
    ensureUserId(sellerUserId, 'sellerUserId'),
    String(adId || '').trim(),
    String(asset || 'USDT').trim().toUpperCase(),
...
function createWalletService(collections, mongoClient, options = {}) {
  ...
  async function findReusableOrder({ session = null, buyerUserId, idempotencyKey = '', flowKey = '' } = {}) {
    const normalizedBuyerUserId = String(buyerUserId || '').trim();
    if (normalizedBuyerUserId && idempotencyKey) {
      const idempotentMatch = await p2pOrders.findOne(
        {
    ...
    if (flowKey) {
      const activeFlowMatch = await p2pOrders.findOne(
        {
          flowKey,
      ...
      if (activeFlowMatch) {
        return {
          ...stripMongoId(activeFlowMatch),
          duplicateOfExisting: true,
          duplicateReason: 'active_flow_guard'
        };
      }
    }
  ...
  async function createEscrowOrder(orderInput) {
    ...
    const orderIdempotencyKey = normalizeIdempotencyKey(
      orderInput.idempotencyKey || orderInput.requestId || orderInput.requestToken
    );
    const orderFlowKey = buildActiveOrderFlowKey({
      buyerUserId,
      sellerUserId,
      adId,
      asset: orderInput.asset || 'USDT',
    ...
    try {
      const result = await runInTransaction(async (session) => {
        ...
        const existingReusableOrder = await findReusableOrder({
          session,
          buyerUserId,
          idempotencyKey: orderIdempotencyKey,
          flowKey: orderFlowKey
        });
        if (existingReusableOrder) {
          return existingReusableOrder;
        }
        ...
        const orderDoc = {
          ...
          flowKey: orderFlowKey,
          chatThreadId: String(orderInput.chatThreadId || orderInput.id || '').trim() || undefined,
          chatReady: true,
          chatReadyAt: now,
          status: 'CREATED',
          createdAt: now,
      ...
      if (error?.code === 11000) {
        const duplicateOrder = await findReusableOrder({
          buyerUserId,
          idempotencyKey: orderIdempotencyKey,
          flowKey: orderFlowKey
        });
  ...
  async function expireOrders(nowMs = Date.now()) {
    ...
    for (const order of candidates) {
      try {
        const expiredOrder = await cancelOrder(
          order.id,
          {
            userId: 'system',
            username: 'System',
    ...
    return { expired, orders: expiredOrders };
  }
