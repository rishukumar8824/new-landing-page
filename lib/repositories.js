...
function createRepositories(collections) {
  ...
  async function updateOffer(offerId, userId, updates = {}) {
    ...
    const normalizedUserId = String(userId || '').trim();
    if (!normalizedOfferId || !normalizedUserId) {
      return null;
    }
    ...
    const result = await p2pOffers.updateOne(
      { id: normalizedOfferId, createdByUserId: normalizedUserId },
      { $set: setFields }
    );
    if (result.matchedCount === 0) {
      return null;
    }
    return getOfferById(normalizedOfferId);
  }
  ...
  async function deleteOffer(offerId, userId) {
    ...
    const result = await p2pOffers.deleteOne({
