...
function createRepositories(collections) {
  ...
  async function ensureSeedOffers(seedOffers) {
        ...
        [
          {
            $set: {
              ...
              merchantDepositLocked: { $ifNull: ['$merchantDepositLocked', true] },
    ...
    if (Array.isArray(seedOffers) && seedOffers.length > 0) {
      const docs = seedOffers.map((offer) => ({
        ...offer,
        status: String(offer.status || 'active').trim().toLowerCase(),
        availableAmount: Number(offer.availableAmount ?? offer.available ?? 0),
        escrowLockedAmount: Number(offer.escrowLockedAmount ?? offer.availableAmount ?? offer.available ?? 0),
        merchantDepositLocked: offer.merchantDepositLocked !== false,
        isDemo: offer.isDemo === true,
        environment: String(offer.environment || 'production').trim().toLowerCase(),
        createdAt: new Date(),
        updatedAt: new Date()
      }));
      await p2pOffers.insertMany(docs, { ordered: false });
      const maxId = await getOfferMaxNumericId();
      await ensureCounter('p2p_offer_seq', maxId);
