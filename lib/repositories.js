  ...
  async function ensureSellerActiveOrderIndex() {
    ...
    if (!hasDesiredIndex) {
      try {
        ...
        const isDuplicateActiveOrderData =
          error?.code === 11000 || /E11000 duplicate key error/i.test(message);

        if (!isDuplicateActiveOrderData) {
          throw error;
        }

        console.warn(
          '[repositories] Skipping unique seller active-order index because duplicate active seller orders already exist. ' +
            'Application-level guards remain active until the legacy data is cleaned up.'
        );
      }
    }
  }
