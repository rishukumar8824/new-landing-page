...
function deriveP2PCredentialIdentity(email, role = 'USER') {
  ...
  return {
    ...
    userId: `usr_${userHash}`,
    username,
    role: normalizeCredentialRole(role)
  };
}
...
function createRepositories(collections) {
  ...
  async function setP2PCredential(email, passwordHash, options = {}) {
    const now = new Date();
    const identity = deriveP2PCredentialIdentity(email, options.role || 'USER');
  ...
  async function touchP2PCredentialLogin(email, loginMeta = {}) {
    const identity = deriveP2PCredentialIdentity(email, loginMeta.role || 'USER');
    const updateDoc = {
      email: identity.email,
      userId: identity.userId,
      username: identity.username,
